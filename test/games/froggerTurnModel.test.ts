...
describe("Frogger refills turn model - sequenced (same-seat) mechanism", () => {
    // Same board/hands as emptyDeckRefills above, but _version bumped past
    // FroggerGame's SEQUENCED_REFILLS_SINCE threshold - games created at or
    // after that version use the new same-seat mechanism (refillPending)
    // instead of the legacy skipto pass-chain, for every refill they ever
    // have. Legacy games (like emptyDeckRefills, version "20251229") keep
    // using skipto forever - see the describe block above.
    ...
    it("a fresh announce keeps currplayer on the announcing seat and sets refillPending, not skipto", () => {
    ...
    it("the supplemental submit happens directly on the same seat - no forced pass from the other seat", () => {
        const g = new FroggerGame(sequencedRefills);
        g.move("c3-b3,NV!/");
    ...
    it("getPlies groups both plies under the same actor and round, with no pass ply between them", () => {
        const g = new FroggerGame(sequencedRefills);
        g.move("c3-b3,NV!/");
        g.move("b3-a2,8MS/");
